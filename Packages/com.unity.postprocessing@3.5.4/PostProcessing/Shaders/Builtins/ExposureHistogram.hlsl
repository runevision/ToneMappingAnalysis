...
float GetAverageLuminance(StructuredBuffer<uint> buffer, float4 params, float maxHistogramValue, float2 scaleOffset)
{
    ...
    float measauredLuminance = filter.x / max(filter.y, EPSILON);

    // Lerp between reference luminance and measured luminance in log space,
    // according to the specified sensitivity.
    float logRef = params.z;
    float logMeasured = log2(measauredLuminance);
    float sensitivity = params.w;
    float logAltered = lerp(logRef, logMeasured, sensitivity);
    return exp2(logAltered);
}
