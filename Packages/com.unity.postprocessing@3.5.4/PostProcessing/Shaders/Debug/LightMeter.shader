Shader "Hidden/PostProcessing/Debug/LightMeter"
{
    HLSLINCLUDE
        ...
        float4 Frag(VaryingsLightMeter i) : SV_Target
        {
            ...
            const float3 kOrigColor = float3(0.0, 0.2, 0.0);
            const float3 kAvgColor = float3(0.75, 0.1, 1.0);

            // Ref brightness marker
            float luminanceRef = GetHistogramBinFromLuminance(exp2(_Params.z), _ScaleOffsetRes.xy);

            if (abs(i.texcoord.x - luminanceRef) < _ScaleOffsetRes.z * 2.0)
            {
            ...
            float origAvg = (luminanceAvg - luminanceRef) / (_Params.w + 0.0001) + luminanceRef;

            if (abs(i.texcoord.x - origAvg) < _ScaleOffsetRes.z * 2.0)
                color.rgb = kOrigColor;
