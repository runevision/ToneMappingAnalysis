...
namespace UnityEngine.Rendering.PostProcessing
{
    ...
    public enum Tonemapper
    {
        ...
        /// <summary>
        /// This method offers a fully parametric, artist-friendly tonemapper that preserves hues.
        /// </summary>
        TonyVibrant
    }
    ...
    public sealed class ColorGrading : PostProcessEffectSettings
    {
        ...
        /// <summary>
        /// The degree to which bright colors become white.
        /// </summary>
        /// <remarks>
        /// This is only used when <see cref="Tonemapper.TonyVibrant"/> is active.
        /// </remarks>
        [DisplayName("Brightness Whitening"), Range(0f, 0.5f), Tooltip("The degree to which bright colors become white.")]
        public FloatParameter whitening = new FloatParameter { value = 0f };
    ...
    internal sealed class ColorGradingRenderer : PostProcessEffectRenderer<ColorGrading>
    {
            ...
            // Lut on the GPU) but every little thing helps, especially on mobile.
            {
                ...
                switch (settings.tonemapper.value)
                {
                    ...
                    case Tonemapper.Custom:
                        ...
                        break;
                    case Tonemapper.TonyVibrant:
                        kernel = compute.FindKernel("KGenLut3D_TonyVibrantTonemap");
                        break;
                }
                ...
                if (settings.tonemapper.value == Tonemapper.Custom || settings.tonemapper.value == Tonemapper.TonyVibrant)
                {
                    m_HableCurve.Init(
                    ...
                    if (settings.tonemapper.value == Tonemapper.TonyVibrant)
                        cmd.SetComputeVectorParam(compute, "_Whitening", Vector4.one * settings.whitening.value);
                }
        ...
        void RenderHDRPipeline2D(PostProcessRenderContext context)
        {
            // For the same reasons as in RenderHDRPipeline3D, regen LUT on every frame
            {
                ...
                if (tonemapper == Tonemapper.Custom || tonemapper == Tonemapper.TonyVibrant)
                {
                    if (tonemapper == Tonemapper.Custom)
                        lutSheet.EnableKeyword("TONEMAPPING_CUSTOM");
                    if (tonemapper == Tonemapper.TonyVibrant)
                        lutSheet.EnableKeyword("TONEMAPPING_TONYVIBRANT");
                    ...
                    lutSheet.properties.SetVector(ShaderIDs.Whitening, Vector4.one * settings.whitening.value);
                }
